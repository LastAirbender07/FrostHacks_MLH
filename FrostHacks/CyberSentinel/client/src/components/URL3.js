      ...
      {error && <div className="text-red-500 mb-8">{error}</div>}
        ...
        <div className="bg-white p-8 rounded-lg">
          <h3 className="mb-4 text-xl font-bold">Scan Result:</h3>
            ...
            <pre
              className="bg-gray-100 p-4 rounded-lg w-[47rem] h-[20rem] max-w-[47rem] max-h-[20rem] overflow-scroll"
              style={{ whiteSpace: 'pre-wrap' }} // Preserve whitespace and wrap text
            >
              {scanResult &&
                scanResult.replace(/\u001b\[(\d{1,2}(;\d{1,2})*)?m/g, '')}
            </pre>
          )}
        </div>
    </div>
